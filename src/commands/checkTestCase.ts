...
export const checkTestCase = async (context: vscode.ExtensionContext) => {
    try {
        ...
        const number = path.dirname(document.fileName).split('/').pop()?.split(':')[0].replace('번', '').trim();
...
const runCommand = async (
    ...
    try {
        ...
        process!.on('close', (code: number) => {
            ...
            if (code === 0) {
                resultConsole.appendLine('-'.repeat(40));
                resultConsole.appendLine(`Test Case ${index + 1}: ${output === resultOutput.trim() ? '성공' : '실패'}`);
                resultConsole.appendLine('-'.repeat(40));
        ...
        resultConsole.appendLine(`테스트 케이스 ${index} 실행 중 오류 발생: ${error}`);
    }
};
