...
import { juseokForm, ProblemHtmlForm, ReadmeForm } from './makeForm';
...
export const makeFolder = async (
    ...
    const folderName =
        platform !== 'Windows_NT' ? `${number}번: ${problemData!.title}` : `${number}번 ${problemData!.title}`;
    ...
    try {
        ...
        try {
            ...
            const ReadmeContent = ReadmeForm(number, problemData!);
            ...
            if (language === 'js') {
                ...
                await vscode.window.showInformationMessage('Input 파일이 생성되었습니다.');
            }
...
const initializeNodeProject = async (projectPath: string) => {
    ...
    try {
        // package.json 파일 생성
        await fs.promises.writeFile(`${projectPath}/package.json`, JSON.stringify(packageJsonContent, null, 2));
